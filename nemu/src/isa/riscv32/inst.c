...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1+imm, 4);if(s->pc==0x800002b8)printf("pc%x %x\n",s->pc,R(rd)));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm); CSR(imm) = src1;printf("src1 %x R(rd)%x\n",src1,R(rd)));
