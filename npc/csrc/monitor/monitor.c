...
void elf_read_fun(char *elf_file) {
	...
	if ((symtab_ind != -1) && (strtab_ind != -1)) {
        int entry_num = sec_headers[symtab_ind].sh_size / sec_headers[symtab_ind].sh_entsize;
		...
		printf("%d\n",entry_num);
		printf("%ld\n",sizeof(Elf32_Sym));
        Elf32_Sym* sym_entries = (Elf32_Sym*)malloc(sizeof(Elf32_Sym)*27);//开辟堆内存用来存储符号表中所有entry
		//char* sym_entries[sizeof(Elf32_Sym)*entry_num];
        //int ret3=fread(sym_entries, sizeof(Elf64_Sym)*entry_num,1, fp);//读符号表
        int ret3=fread(sym_entries,sizeof(Elf32_Sym)*27,1, fp);
		assert(ret3==1);
